...
app.post('/api/deep-dive', async (req, res) => {
  try {
    const { playerName, team, opponent, statType, line, seasonAvg, last5, oppInjuries } = req.body || {};
    if (!playerName) return res.status(400).json({ success: false, error: 'No player' });
    if (!process.env.ANTHROPIC_API_KEY) return res.json({ success: true, analysis: 'Add ANTHROPIC_API_KEY to unlock AI deep dive analysis.' });

    const injCtx = (oppInjuries || []).map(i => i.playerName + ' (' + i.status + ' - ' + i.injury + ')').join(', ');
    const last5Ctx = (last5 || []).map(g => g.date + ': ' + g.pts + 'pts/' + g.reb + 'reb/' + g.ast + 'ast vs ' + (g.matchup || '?')).join('; ');

    const prompt = `Analyze this NBA player prop for betting:
Player: ${playerName} (${team}) vs ${opponent}
Prop: ${statType} ${line}
Season Avg: ${seasonAvg || 'unknown'}
Last 5 games: ${last5Ctx || 'unavailable'}
${opponent} injuries: ${injCtx || 'none reported'}

Give a sharp 100-word max betting analysis covering: matchup advantage/disadvantage, pace impact, injury effect on usage, trend direction, and a clear OVER/UNDER/SKIP verdict with confidence. Be specific with numbers. Format with bullet points.`;

    const { data } = await axios.post('https://api.anthropic.com/v1/messages', {
      model: 'claude-haiku-4-5-20251001', max_tokens: 300,
      system: 'You are a sharp NBA betting analyst. Be concise, data-driven, and opinionated. Always give a clear verdict.',
    ...
    res.json({ success: true, analysis: 'Analysis temporarily unavailable. Check back shortly.' });
  }
});

// ── PROJECTIONS WITH REAL DATA ──
app.get('/api/projections', async (req, res) => {
  try {
    ...
    const r = await axios.get('https://api.balldontlie.io/v1/stats', {
      headers: { Authorization: BDL_KEY },
      params: { 'player_ids[]': found.id, 'seasons[]': 2025, per_page: 20, postseason: false },
      timeout: 8000
    });
    const all = (r.data.data || []).sort((a, b) => (b.game?.date || '').localeCompare(a.game?.date || ''));
    ...
    const statMap = { points: 'pts', rebounds: 'reb', assists: 'ast', steals: 'stl', blocks: 'blk' };
    const field = statMap[statType] || 'pts';

    const avg10 = last10.length ? (last10.reduce((s, g) => s + (g[field] || 0), 0) / last10.length).toFixed(1) : null;
    const avg5 = last5.length ? (last5.reduce((s, g) => s + (g[field] || 0), 0) / last5.length).toFixed(1) : null;
    const median10 = last10.length ? last10.map(g => g[field] || 0).sort((a, b) => a - b)[Math.floor(last10.length / 2)] : null;

    // Trend: compare last 5 to last 10
    const trending = avg5 && avg10 ? (parseFloat(avg5) > parseFloat(avg10) ? 'up' : parseFloat(avg5) < parseFloat(avg10) ? 'down' : 'flat') : 'unknown';

    res.json({
      success: true, projection: {
        avg10: parseFloat(avg10), avg5: parseFloat(avg5), median10,
        trending, games: last10.length,
        last5: last5.map(g => {
          const gm = g.game || {};
          return { date: (gm.date || '').split('T')[0], val: g[field] || 0, pts: g.pts, reb: g.reb, ast: g.ast, min: g.min ? parseInt(g.min) : 0 };
        })
      }
    });
  } catch (e) {
